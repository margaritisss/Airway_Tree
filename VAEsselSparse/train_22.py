...
from vaessel_22 import Encoder, Decoder, to_sp
from data_22 import VesselDataset, list_segmentations
...
EPOCHS       = 1000
LR           = 1e-4
BETA         = 1e-3         # KL weight; the paper does not specify a value -> tune this
...
def evaluate(enc, dec, loader, grid):
    ...
    for batch in loader:
        ...
        x = to_sp(ts, spatial_range=(1, *grid))   # use the `grid` arg, val batch size is 1
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        for it, batch in enumerate(train_loader):
            ...
            x  = to_sp(ts, spatial_range=(BATCH_SIZE, *GRID))   # was: to_sp(ts)
