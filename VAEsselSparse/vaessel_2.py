...
class DecoderStage(nn.Module):
    """Eq. 4:  Y~ = Up(Y);  Y' = ResSparseBlock(Y~).  Doubles resolution once.

    `Up` = (channel-projection conv at the coarse res) -> sp.SparseSubdivide()
    (generative 2x: each voxel -> its 2x2x2 children). Subdivide grows coords
    (*2 + offsets) with NO clamp and NO spatial_range dependency -- unlike the
    torchsparse generative transposed conv, which on this build clamps the
    upsampled coords to a *downsampled* spatial_range and collapses the volume
    (verified: extent 79->39->19->9 instead of 79->159->319->639).
    """
    def __init__(self, in_channels, out_channels, num_groups=8):
        super().__init__()
        self.pre = nn.Sequential(                              # project channels @ coarse res
            sp.SparseConv3d(in_channels, out_channels, 3, padding=1),
            sp.SparseGroupNorm(num_groups, out_channels),
            sp.SparseReLU(),
        )
        self.subdivide = sp.SparseSubdivide()                 # Up: 2x resolution, keeps channels
        self.res = ResSparseBlock(out_channels, out_channels, num_groups=num_groups)

    def forward(self, x):
        return self.res(self.subdivide(self.pre(x)))          # Y~ = Up(Y); Y' = ResSparseBlock(Y~)
