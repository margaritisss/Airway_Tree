...
def _deconv_block(in_ch: int, out_ch: int, kernel_size: int = 3, stride: int = 1, padding: int = 1, output_padding: int = 0, activation: bool = True) -> nn.Sequential:
    ...
    layers = [ nn.ConvTranspose3d(in_ch, out_ch, kernel_size=kernel_size, stride=stride, padding=padding, output_padding=output_padding, bias=False),
               nn.BatchNorm3d(out_ch),]
...
class VoxelVAE128(nn.Module):
    ...
    def __init__(self, num_latents: int = 100, n_channels: int = 1, base_ch: int = 4, fc_dim: int = 343):
        ...
        c7  = base_ch * 64     # 512
        c8  = base_ch * 128    # 1024
        c9  = base_ch * 256    # 1024   (new)
        c10 = base_ch * 512    # 2048   (new)
        ...
        self.enc9  = _conv_block(c8, c9,  kernel_size=3, stride=1, padding=0)         # 15 -> 13   | Ch: c8 -> c9
        ...
        # Note: We halve the channels at every step as we work our way back up
        self.dec1  = _conv_block(  1, c10, kernel_size=3, stride=1, padding=1)        # 7 -> 7     | Ch: 1 -> c10 (Maintain start size)
        self.dec2  = _deconv_block(c10, c9, kernel_size=3, stride=2, padding=0)       # 7 -> 15    | Ch: c10 -> c9
        self.dec3  = _conv_block(  c9, c8, kernel_size=3, stride=1, padding=1)        # 15 -> 15   | Ch: c9 -> c8
        self.dec4  = _deconv_block(c8, c7, kernel_size=3, stride=2, padding=0)        # 15 -> 31   | Ch: c8 -> c7
        self.dec5  = _conv_block(  c7, c6, kernel_size=3, stride=1, padding=1)        # 31 -> 31   | Ch: c7 -> c6
        self.dec6  = _deconv_block(c6, c5, kernel_size=3, stride=2, padding=0)        # 31 -> 63   | Ch: c6 -> c5
        self.dec7  = _conv_block(  c5, c4, kernel_size=3, stride=1, padding=1)        # 63 -> 63   | Ch: c5 -> c4
        self.dec8  = _deconv_block(c4, c3, kernel_size=3, stride=2, padding=0)        # 63 -> 127  | Ch: c4 -> c3
        self.dec9  = _conv_block(  c3, c2, kernel_size=3, stride=1, padding=1)        # 127 -> 127 | Ch: c3 -> c2
        self.dec10 = _deconv_block(c2, c1, kernel_size=4, stride=2, padding=0)        # 127 -> 256 | Ch: c2 -> c1 (KERNEL=4!)

        # Final layer: standard convolution to map down to output channel (1)
        self.dec11 = nn.Sequential( nn.Conv3d(c1, n_channels, kernel_size=3, stride=1, padding=1, bias=False),nn.BatchNorm3d(n_channels))
        self._init_weights()
